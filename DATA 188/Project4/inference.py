...
def constructBayesNet(gameState: hunters.GameState):
    ...
    obs_domain = list(range(max_distance + MAX_NOISE + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elimVar in eliminationOrder:
            currentFactorsList, joinFactor = joinFactorsByVariable(currentFactorsList, elimVar)
            if len(joinFactor.unconditionedVariables()) > 1:
                eliminatedFactor = eliminate(joinFactor, elimVar)
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            numPosDist = self.getPositionDistribution(gameState, oldPos)
            newParticle = numPosDist.sample()
