    ...
    def runValueIteration(self):
        ...
        for s in self.mdp.getStates():
            if not self.mdp.isTerminal(s):
                poss_actions = self.mdp.getPossibleActions(s)
                if not poss_actions:
                    max_q_value = 0
                else:
                    q_values = [self.computeQValueFromValues(s, action) for action in poss_actions]
        ...
        for i in range(self.iterations):
            ...
            for p in predecessors[s]:
                poss_actions_p = self.mdp.getPossibleActions(p)
                if not poss_actions_p:
                    max_q_value_p = 0
                else:
                    q_values_p = [self.computeQValueFromValues(p, action) for action in poss_actions_p]
